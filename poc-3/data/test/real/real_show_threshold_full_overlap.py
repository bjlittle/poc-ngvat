...
pdata_sst = {t: pv.read(f"pdata_xy_sst_t{t}.vtk") for t in range(N_steps)}
sst = pv.read("pdata_xy_sst_t0.vtk")
sst.set_active_scalars("faces")

sst_threshold = pv.read("pdata_xy_sst_t0.vtk")
idx = np.arange(sst_threshold.n_cells)
sst_threshold.cell_arrays["idx"] = idx
outside = sst_threshold.threshold(**tkwargs)
sst_threshold["idx"][outside["idx"]] = -1
idx = sst_threshold["idx"]
sst_threshold = sst_threshold.threshold(value=0, scalars="idx")
sst_threshold.set_active_scalars("faces")

pdata_sst_threshold = {}
for t in range(N_steps):
    mesh = pv.read(f"pdata_xy_sst_t{t}.vtk")
    mesh.cell_arrays["idx"] = idx
    pdata_sst_threshold[t] = mesh.threshold(value=0, scalars="idx")
